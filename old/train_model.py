...
def main_loop(model, train_ds, epochs=3, inner_lr=0.01, batch_size=4, log_steps=1000, config=None):
    ...
    for epoch in range(epochs):
        ...
        for i, batch_ds in enumerate(random.sample(train_ds, len(train_ds))):
            ...
            #              support_y [4, 5*1, 5]
            batch_support_x = tf.slice(x, [0,0,0], [-1, n_way * k_shot, -1], name='support_x')
            batch_query_x   = tf.slice(x, [0, n_way * k_shot, 0], [-1, -1, -1], name='query_x')
            # support set: support_x [4, 5*15, 84*84*3]
            #              support_y [4, 5*15, 5]
            batch_support_y = tf.slice(y, [0,0,0], [-1, n_way * k_shot, -1], name='support_y')
            batch_query_y   = tf.slice(y, [0, n_way * k_shot, 0], [-1, -1, -1], name='query_y')
